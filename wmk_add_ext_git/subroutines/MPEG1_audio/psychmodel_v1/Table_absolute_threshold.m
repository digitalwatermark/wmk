...
% accelerated and one bug fixed
%-------------------------------------------------------------------------------

function [TH, Map, LTq] = Table_absolute_threshold(Layer, fs, bitrate, NFFT)
...
% Revised by Zhang Peng, Tsinghua Univ, 2009-02-27.
% 
...
%       figure; plot(TH(:,1),TH(:,2)); grid; xlabel('freq / Hz'); ylabel('critical band rate/ bark'); title('frequency vs. critical band rate');
%       figure; plot(TH(:,1),TH(:,3)); grid; xlabel('freq / Hz'); ylabel('absolute threshold / dB'); title('frequency vs. absolute threshold');
      
%       N = length(TH(:, 1));
      
      % Convert frequencies to samples indecies.
%       for i = 1:N,
%          TH(i, INDEX) = round(TH(i, INDEX) / fs * NFFT); 
%       end
    
TH(:, INDEX) = round(TH(:, INDEX) / fs * NFFT);
      
      % Generate a mapping between the NFFT / 2 samples of the input
...
%          Map(j) = 1;
%       end
%       for j = TH(N, INDEX):NFFT/2,
%          Map(j) = N;
%       end
%       % All the other (from table)
%       for i = 2:N-1,                                          % bug. should be i=1:N-1
...
Map = zeros(NFFT/2, 1);
Map(1:TH(1, INDEX)) = 1;
Map(TH(end, INDEX): end) = size(TH,1);
for ii=1:size(TH,1)-1
    Map(TH(ii, INDEX):TH(ii+1, INDEX)-1) = ii;
end
