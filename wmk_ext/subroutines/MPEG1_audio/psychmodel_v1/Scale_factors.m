...
%    else
%       j = 0;
%       while (j < N & si_min > Table_scf(N - j))     % 不小于si_min的最小因子
%          j = j + 1;
%       end
%       scf(i) = Table_scf(N - j);     
%    end
...
si_min = max(abs(S), [], 1);
for ii=1:32
    idx = find(Table_scf>=si_min(ii), 1, 'last');       % 不小于si_min的最小因子
    if isempty(idx)
        idx = 1;
        warning('Cannot find scale factor.');
    end
    scf(ii) = Table_scf(idx);
end

% if max(abs(scf-scf2))~=0
%     disp('*********');
% end