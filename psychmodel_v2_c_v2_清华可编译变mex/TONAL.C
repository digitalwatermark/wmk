...
int lay, freq;              /* band boundaries                 */
{
 ...
 // pointers of CB tables
 int *pt_cbtab[2][3] = {{CritBand_1CB0, CritBand_1CB1, CritBand_1CB2},
												{CritBand_2CB0, CritBand_2CB1, CritBand_2CB2}};
 ...
 */
 ...
 crit_band = pt_cbtab[lay-1][freq][0];
 ...
 */
 ...
 for(i=0;i<crit_band;i++)
	 cbound[i] = pt_cbtab[lay-1][freq][i+1];


 /*
 fp = fopen("cb.txt", "wt");	// generate table
 fprintf(fp, "%d,\n", crit_band);
 for (i=0;i<crit_band;i++) {
	 fprintf(fp, "%d, ", cbound[i]);
...
g_ptr FAR *ltg;                /* values                   */
{
 ...
 // pointers of CB tables
 double *pt_thtab[2][3] = {
					{&AbsThr_1TH0[0][0], &AbsThr_1TH1[0][0], &AbsThr_1TH2[0][0]},
					{&AbsThr_2TH0[0][0], &AbsThr_2TH1[0][0], &AbsThr_2TH2[0][0]}};
 ...
 */
 ...
 sub_size = (int) (pt_thtab[lay-1][freq][0]);
 ...
 *ltg = (g_ptr FAR ) mem_alloc(sizeof(g_thres) * sub_size, "ltg");
 ...
 for(i=1;i<sub_size;i++) {
	 (*ltg)[i].line = (int) (pt_thtab[lay-1][freq][i*3+0]);
	 (*ltg)[i].bark = pt_thtab[lay-1][freq][i*3+1];
	 (*ltg)[i].hear = pt_thtab[lay-1][freq][i*3+2];
 }
    ...
    weight += pow(10.0,power[j].x/10.0) * (double) (j-cbound[i]) /
     ...
     if(sum <= DBMIN)  centre = (cbound[i+1]+cbound[i]) /2;
        ...
        index = weight/pow(10.0,sum/10.0);
				///////////////////////////////////////////////
				// +1e-14 modified by Peng Zhang, 2010-03-10, to force the result of Matlab
				// to be the same as C after some non-linear operations on double numbers
				// such as (int)
        centre = cbound[i] + (int) (index * (double) (cbound[i+1]-cbound[i])+1e-14 );
     } 
#else
...
void I_Psycho_One(buffer1, scale, ltmin, fr_ps)
//short FAR buffer[2][1152];
double buffer1[2][1152];
double FAR scale[2][SBLIMIT], ltmin[2][SBLIMIT];
frame_params *fr_ps;
{
...
int stereo = fr_ps->stereo;
 ...
 for(k=0;k<1;k++){  //  /* check PCM input for a block of 
    for(i=0;i<384;i++)     ///* 384 samples for a 512-pt. FFT  
	{ fft_buf[k][(i+off[k])%640]= buffer1[k][i];
//		fprintf(ltmintest,"index = %d\n",(i+off[k])%640);
	}
    for(i=0;i<FFT_SIZE/2;i++)
	{
		sample[i] = fft_buf[k][(i+448+off[k])%640];
...
//	fwrite(ltmin[0],sizeof(double),32,ltmintest);
