...
/* initialization time.  The mem_free() function must not be called.      */
     ...
     //read_absthr(absthr, sfreq_idx);
		 ...
		 /*		 
		 fp = fopen("absthr.txt", "wt");	// generate table
		 for (i=0;i<HBLKSIZE;i++) {
			 fprintf(fp, "%.2f, ", absthr[i]);
			 if (i%4==3)	 fprintf(fp, "\n");
		 }
		 fclose(fp);
		 */
...
/*temp1 is the preliminary threshold */
           ...
           //temp1=(temp1>absthr[j])?temp1:absthr[j];
					 ...
					 temp1=(temp1>AbsThr[sfreq_idx][j])?temp1:AbsThr[sfreq_idx][j];
...
******************************************************************************/
// bug fixed by Peng Zhang, 2010-03-09
// change "float a" to "double a", and use sscanf(t,"%lf", &a) instead of "%f"
/*
