    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->NUMComboBox->addItem("id1");
    ui->NUMComboBox->addItem("id2");
    ui->NUMComboBox->addItem("id3");
    ui->NUMComboBox->addItem("id4");
    ui->NUMComboBox->addItem("id5");
    ui->NUMComboBox->addItem("id6");

    ui->NAMEComboBox->addItem("id1");
    ui->NAMEComboBox->addItem("id2");
    ui->NAMEComboBox->addItem("id3");
    ui->NAMEComboBox->addItem("id4");
    ui->NAMEComboBox->addItem("id5");
    ui->NAMEComboBox->addItem("id6");
    ...
    ui->NAMEComboBox->setVisible(false);
    ui->NUMComboBox->setVisible(false);
...
{   
    ...
    string Input_Text = input_text.toStdString();
    input_lenght = Input_Text.length();
    input_txt = new char[input_lenght+1];
    strcpy(input_txt,Input_Text.c_str());

    input_txt_int = new int[input_lenght+1];
    //input_txt_int[0] = 36;
    input_txt_int[0] = 0;

    for(i=0; i<input_lenght; i++)
    {
        /*if(input_txt[i]>='0'&& input_txt[i]<='9')
            input_txt_int[i+1] = int(input_txt[i])-48;
        else if(input_txt[i]>='a'&& input_txt[i]<='z')
            input_txt_int[i+1] = int(input_txt[i])-87;*/

        if(input_txt[i]>='a'&& input_txt[i]<='d')
            input_txt_int[i+1] = int(input_txt[i])-96;
        else
            MessageBox(NULL,TEXT("Please input a~d."),TEXT("Warning"),MB_OK);
...
{
    WMAdd = checked;
    if (checked == true && AnalogOrRead == true)
    {
...
void MainWindow::on_Analog_radioButton_clicked(bool checked)//声卡输入输出
{
    if(ifisrunning)
    {
        MessageBox(NULL,TEXT("It's running now!You can push the 'stop' button first!"),TEXT("Warning"),MB_OK);
        ui->Analog_radioButton->setChecked(false);
        ui->File_radioButton->setChecked(true);
    }
    else
    {
