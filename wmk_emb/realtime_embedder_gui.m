% % Real-time Watermark Embedder (GUI)
% % Peng Zhang    Tsinghua Univ.    2010.4.25
% % % % % % % % % % % % % % % % % % % 

function varargout = realtime_embedder_gui(varargin)
%fprintf('=====================realtime_embedder_gui=====================\n');
% REALTIME_EMBEDDER_GUI M-file for realtime_embedder_gui.fig
%      REALTIME_EMBEDDER_GUI, by itself, creates a new REALTIME_EMBEDDER_GUI or raises the existing
%      singleton*.
%
%      H = REALTIME_EMBEDDER_GUI returns the handle to a new REALTIME_EMBEDDER_GUI or the handle to
%      the existing singleton*.
%
%      REALTIME_EMBEDDER_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in REALTIME_EMBEDDER_GUI.M with the given input arguments.
%
%      REALTIME_EMBEDDER_GUI('Property','Value',...) creates a new REALTIME_EMBEDDER_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before realtime_embedder_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to realtime_embedder_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help realtime_embedder_gui

% Last Modified by GUIDE v2.5 17-Jul-2011 17:42:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @realtime_embedder_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @realtime_embedder_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before realtime_embedder_gui is made visible.
function realtime_embedder_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to realtime_embedder_gui (see VARARGIN)

% Choose default command line output for realtime_embedder_gui
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes realtime_embedder_gui wait for user response (see UIRESUME)
...
Input_Gain = 1;
Fs = 48000;
Play_Mode = 'once';   % 'Once', 'Repeat', or 'Repeat all'
Sync_Word = '0d';
Wmk_Format = 'text';          % 'text', 'bin', or 'mat'
Bit_Format = 'bin';
N_CharPerLine = 16;
Plot_Enable = [1 1];
Plot_IO = 1;            % 1: input; 0: output
...
% not adjustable through GUI
Hplot = [findobj('tag', 'axes_wavform'); findobj('tag', 'axes_psd')];
Npad_Prefix = 128;
Npad_Suffix = 128;
Obj_Handle = struct();
Obj_Handle.Slider = findobj('tag', 'slider_progress');
Obj_Handle.CurrentWav = findobj('tag', 'text_current_wav');
Obj_Handle.TotalTime = findobj('tag', 'text_total_time');
Obj_Handle.CurrentTime = findobj('tag', 'text_current_time');

% clear and initialize
Current_Bit_Index = 1;
Nframe_In_Buffer = 1;
Current_Wav_Info = struct('Fs', [], 'Nbit', [], 'Size', [], 'DurationString', '');
...
if DA_Polar==1
    set(findobj('tag', 'popupmenu_dapolar'), 'value', 1);
elseif DA_Polar==-1
    set(findobj('tag', 'popupmenu_dapolar'), 'value', 2);
else
    error('D/A polar error.');
end

if Mic_Enable
    set(findobj('tag', 'radiobutton_file_on'), 'value', 0);
    set(findobj('tag', 'radiobutton_mic_on'), 'value', 1);
    set(findobj('tag', 'uipanel_mic'), 'visible', 'on');
    set(findobj('tag', 'uipanel_file'), 'visible', 'off');
    fname = fieldnames(Obj_Handle);
    for ii=1:length(fname)
        set(getfield(Obj_Handle, fname{ii}), 'visible', 'off')
    end
else
    set(findobj('tag', 'radiobutton_file_on'), 'value', 1);
    set(findobj('tag', 'radiobutton_mic_on'), 'value', 0);
    set(findobj('tag', 'uipanel_mic'), 'visible', 'off');
    set(findobj('tag', 'uipanel_file'), 'visible', 'on');
    fname = fieldnames(Obj_Handle);
    for ii=1:length(fname)
        set(getfield(Obj_Handle, fname{ii}), 'visible', 'on')
    end
end
...
if strcmpi(Play_Mode, 'Once')
    set(findobj('tag', 'popupmenu_playmode'), 'value', 1);
elseif strcmpi(Play_Mode, 'Repeat')
    set(findobj('tag', 'popupmenu_playmode'), 'value', 2);
elseif strcmpi(Play_Mode, 'Repeat All')
    set(findobj('tag', 'popupmenu_playmode'), 'value', 3);
else
    error('Play mode error.');
end
...
set(findobj('tag', 'radiobutton_display_input'), 'value', Plot_IO);
set(findobj('tag', 'radiobutton_display_output'), 'value', ~Plot_IO);
...
set(findobj('tag', 'radiobutton_embed_bypass'), 'value', Wmk_Bypass);
set(findobj('tag', 'radiobutton_embed_enable'), 'value', ~Wmk_Bypass);
if Wmk_Bypass
    set(findobj('tag', 'listbox_wmk'), 'enable', 'off');
    set(findobj('tag', 'edit_wmk_info'), 'string', '  No Watermark');
else
    set(findobj('tag', 'listbox_wmk'), 'enable', 'on');
    set(findobj('tag', 'edit_wmk_info'), 'string', '');
end


% --- Outputs from this function are returned to the command line.
function varargout = realtime_embedder_gui_OutputFcn(hObject, eventdata, handles)
...
% --- Executes on button press in radiobutton_file_on.
function radiobutton_file_on_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_file_on (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_file_on
globalvar_realtime_wmkembedder;
if ~isempty(Ai) || ~isempty(Ao)    
    stop([Ai, Ao]);
end
...
% --- Executes on button press in radiobutton_mic_on.
function radiobutton_mic_on_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_mic_on (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_mic_on
globalvar_realtime_wmkembedder;
if ~isempty(Ai) || ~isempty(Ao)    
    stop([Ai, Ao]);
end
...
set(findobj('tag', 'popupmenu_disp_format'), 'enable', 'off');
if ~strcmpi(Wmk_Format, 'text')
    Wmk_Text = bit2text(Wmk_Bit, Sync_Word);
    set(findobj('tag', 'edit_wmk'), 'string', Wmk_Text);        
end
Wmk_Format = 'text';
...
set(findobj('tag', 'popupmenu_disp_format'), 'enable', 'on');
if strcmpi(Wmk_Format, 'text')
    Wmk_Text = bit2strcell(Wmk_Bit, N_CharPerLine, Bit_Format);
    set(findobj('tag', 'edit_wmk'), 'string', Wmk_Text);
end
Wmk_Format = 'bin';
...
% --- Executes on selection change in popupmenu_dapolar.
function popupmenu_dapolar_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenu_dapolar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns popupmenu_dapolar contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_dapolar
globalvar_realtime_wmkembedder;
index = get(gcbo, 'value');
str = get(gcbo, 'string');
DA_Polar = str2num(str{index});


% --- Executes during object creation, after setting all properties.
function popupmenu_dapolar_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenu_dapolar (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in popupmenu_playmode.
function popupmenu_playmode_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenu_playmode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns popupmenu_playmode contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_playmode
globalvar_realtime_wmkembedder;
index = get(gcbo, 'value');
str = get(gcbo, 'string');
Play_Mode = str{index};
if strcmpi(Play_Mode, 'repeat all')   
    Selected_Wav_Index = 1:length(Files_Wav);
    set(findobj('tag', 'src_audio_list'), 'value', Selected_Wav_Index);
    set(findobj('tag', 'src_audio_list'), 'enable', 'off');
else
    set(findobj('tag', 'src_audio_list'), 'enable', 'on');
end
Current_Wav_Index = 1;
Current_Sample_Index = 1;
    
% --- Executes during object creation, after setting all properties.
function popupmenu_playmode_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenu_playmode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
Wmk_Bit = text2bit(Wmk_Text, Sync_Word);
...
function pushbutton_sel_src_au_Callback(hObject, ~, handles)
...
%==========================writewavhead=====================================
fileclosename = sprintf('%s%swithwm.wav',pathname,filename(1,1:end-4));
outfile = fopen(fileclosename,'w');
infile = fopen(sprintf('%s%s',pathname,filename));

headoffile =  fread(infile,1,'int32');
while( headoffile~=1635017060)%chartmp2~=97 || chartmp3~=116  || chartmp4~=97
    fwrite(outfile,headoffile,'int32');
    headoffile =  fread(infile,1,'int32');
end;
fwrite(outfile,headoffile,'int32');
lengthofnew = fread(infile,1,'int32');
fwrite(outfile,lengthofnew,'int32');%floor(lengthofnew/32768)*32768
writesignal = 1;
fclose(infile);
%==========================writewavheadend=====================================
if isnumeric(fileclosename)               % click 'cancel'
    return
end
...
if strcmpi(Wmk_Format, 'text')
    ...
    [Wmk_Bit, syncbit, Wmk_Text] = text2bit(Files_Watermark, Sync_Word, '', max_lineread);
...
%     Wmk_Text = mat2cell(str, ones(size(str,1),1), size(str,2));

    Wmk_Bit = dec2binstream(dec8bit, 8, 'msb');
    Wmk_Text = bit2strcell(Wmk_Bit, N_CharPerLine, Bit_Format);
...
elseif strcmpi(Wmk_Format, 'mat')
    ...
    Wmk_Bit = eval(var_name);
    Wmk_Bit = Wmk_Bit(1:min(max_byteread*8, size(Wmk_Bit, 1)), :);
    nbyte_ch = ceil(size(Wmk_Bit, 1)/8);
    nchan = size(Wmk_Bit, 2);
    Wmk_Bit = [Wmk_Bit; zeros(nbyte_ch*8-size(Wmk_Bit, 1), nchan)];       % pad zeros

%     if nchan ==2
%         temp = reshape(Wmk_Bit(:), 8, nbyte_ch*nchan);
...
globalvar_realtime_wmkembedder;
Wmk_Text = get(gcbo, 'string');
if isempty(Wmk_Text{1})
    Wmk_Bit = [];
    return
end
if strcmpi(Wmk_Format, 'text')
    Wmk_Bit = text2bit(Wmk_Text, Sync_Word);
%     bit2text(Wmk_Bit, Sync_Word)
elseif strcmpi(Wmk_Format, 'bin') || strcmpi(Wmk_Format, 'mat')   
%     temp = cell2mat(Wmk_Text);                % hex
%     nchan = size(temp, 2)/2;
%     if nchan==2                                              % dec
...
%     Wmk_Bit = reshape(temp, length(temp)/nchan, nchan);
      
    Wmk_Bit = strcell2bit(Wmk_Text, Bit_Format);
end
...
globalvar_realtime_wmkembedder; 
if ~isempty(get(gcbo, 'value'))
    Selected_Wav_Index = get(gcbo, 'value');
    Current_Wav_Index = Selected_Wav_Index(1);
    Current_Sample_Index = 1;
...
globalvar_realtime_wmkembedder;
Mic_Enable=0;%++++++++++++++
if ~Mic_Enable && isempty(Files_Wav)
    errordlg('No wav file specified.', 'No file', 'replace');
    return
end   

if isempty(Wmk_Bit) && strcmpi(Wmk_Type, 'extractable')
    warndlg('No watermark specified. Host audio played.', 'No watermark', 'replace');
end

% get default parameters
% [wmk_temp, Algorithm_Param] = wmk_core_ccsk_v2(0, Fs, [], [], Key, 'initial'); 
...
% Len_Buffer = Nframe_In_Buffer*Algorithm_Param.N;
Len_Buffer = Algorithm_Param.N + Npad_Prefix + Npad_Suffix;
Signal_Buffer = zeros(Len_Buffer, 2);
Pnmod_Buffer = zeros(Len_Buffer, 2);
...
%     Current_Wav_Info.Size = wavread(Files_Wav{Current_Wav_Index}, 'size');
     [ Current_Wav_Info.y,Fs] = audioread(Files_Wav{Current_Wav_Index});
end
...
% --- Executes on button press in pushbutton_volctrl.
function pushbutton_volctrl_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_volctrl (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% ! sndvol32&
...
% t = timer('Period',0.150, 'ExecutionMode','fixedRate','Tag', 'timer_count_utl_snd_rdy');
t = timer('Tag', 'timer_count_utl_snd_rdy', 'Period',0.150, 'ExecutionMode','fixedRate');
t.StartDelay = 0;
t.TasksToExecute = ceil(length(Current_Wav_Info.y(:,1))/Algorithm_Param.N);
set(t,'TimerFcn',{'acquire_embed',Algorithm_Param.N, nfft, Algorithm_Param,Current_Wav_Info.y});
start(t);
...
globalvar_realtime_wmkembedder;
if isempty(Files_Wav) || isempty(Current_Wav_Index)
    set(gcbo, 'value', 0);
    return
end
v = get(gcbo, 'value');
Current_Sample_Index = min(round(Current_Wav_Info.Size(1)*v)+1, Current_Wav_Info.Size(1)-1);
...
% --- Executes on selection change in text_current_wav.
function text_current_wav_Callback(hObject, eventdata, handles)
% hObject    handle to text_current_wav (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns text_current_wav contents as cell array
%        contents{get(hObject,'Value')} returns selected item from text_current_wav


% --- Executes during object creation, after setting all properties.
function text_current_wav_CreateFcn(hObject, eventdata, handles)
% hObject    handle to text_current_wav (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in radiobutton_display_input.
function radiobutton_display_input_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_display_input (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_display_input
globalvar_realtime_wmkembedder;
Plot_IO = 1;
set(gcbo, 'value', 1);
set(findobj('tag', 'radiobutton_display_output'), 'value', ~Plot_IO);

% --- Executes on button press in radiobutton_display_output.
function radiobutton_display_output_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_display_output (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_display_output
globalvar_realtime_wmkembedder;
Plot_IO = 0;
set(gcbo, 'value', 1);
set(findobj('tag', 'radiobutton_display_input'), 'value', Plot_IO);
...
% handles    structure with handles and user data (see GUIDATA)
globalvar_realtime_wmkembedder;
str = sprintf('Real-time Audio Watermark Embedder v3.0\nPeng Zhang,  Tsinghua Univ.  Jul. 2011\nAll rights reserved.');
logofile = 'logo.gif';
if exist(logofile)==2
...
% --- Executes on button press in radiobutton_embed_enable.
function radiobutton_embed_enable_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_embed_enable (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_embed_enable
globalvar_realtime_wmkembedder;
set(gcbo, 'value', 1);
Wmk_Bypass = 0;
set_detectable_wmk();
...
% --- Executes on button press in radiobutton_embed_bypass.
function radiobutton_embed_bypass_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton_embed_bypass (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton_embed_bypass
globalvar_realtime_wmkembedder;
set(gcbo, 'value', 1);
Wmk_Bypass = 1;
set_detectable_wmk();
...
set(findobj('tag', 'edit_wmk_info'), 'string', disp_str);
set(findobj('tag', 'radiobutton_embed_bypass'), 'value', Wmk_Bypass);
set(findobj('tag', 'radiobutton_embed_enable'), 'value', ~Wmk_Bypass);
