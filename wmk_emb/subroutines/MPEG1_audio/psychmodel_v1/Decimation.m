...
%     end   
    ...
    % new code (accelerated)
    idx = Non_tonal_list(:, SPL) < TH(Map(Non_tonal_list(:, INDEX)), ATH);  % faster than find()
    DFlags(Non_tonal_list(idx, INDEX)) = IRRELEVANT;
    DNon_tonal_list = Non_tonal_list(not(idx), :);
%     idx = find(Non_tonal_list(:, SPL) < TH(Map(Non_tonal_list(:, INDEX)), ATH));
%     DNon_tonal_list = Non_tonal_list(setdiff(1:length(Non_tonal_list(:, 1)), idx), :);  % setdiff slow    
...
%     end
    ...
    % new code (accelerated)
    idx = Tonal_list(:, SPL) < TH(Map(Tonal_list(:, INDEX)), ATH);  % faster than find()
    DFlags(Tonal_list(idx, INDEX)) = IRRELEVANT;
    DTonal_list = Tonal_list(not(idx), :);
